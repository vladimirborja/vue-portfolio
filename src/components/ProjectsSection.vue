...
<template>
    <section id="projects" v-scroll-reveal class="py-24 border-t border-green-900/40">
        <div class="max-w-5xl mx-auto px-6">
            ...
            <h2 class="text-3xl font-bold text-white mb-12">Things I've built</h2>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div v-for="project in projects" :key="project.name"
                    class="border border-green-900/60 bg-green-950/10 rounded-lg overflow-hidden flex flex-col justify-between hover:border-green-600/50 transition-colors duration-300">
                    <div class="w-full h-40 bg-green-950/30 border-b border-green-900/60 overflow-hidden">
                        <img :src="project.image" :alt="`${project.name} landing page`"
                            class="w-full h-full object-cover"
                            onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'" />
                        <span
                            class="hidden w-full h-full items-center justify-center font-mono-custom text-xs text-green-700 text-center px-4">
                        </span>
                    </div>
                ...
                <div class="border border-dashed border-green-900/40 rounded-lg p-6 flex items-center justify-center">
