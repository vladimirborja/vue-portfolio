<script setup>
const projects = [
    {
        name: 'vue-portfolio',
        desc: 'Personal portfolio built with Vue 3 and TailwindCSS. Dark theme with terminal aesthetics.',
        tags: ['Vue 3', 'TailwindCSS', 'Vite'],
        repo: 'https://github.com/yourusername/vue-portfolio',
        live: '#',
    },
    {
        name: 'project-two',
        desc: 'A short description of what this project does and what problem it solves.',
        tags: ['Node.js', 'Express', 'PostgreSQL'],
        repo: 'https://github.com/yourusername',
        live: '#',
    },
    {
        name: 'project-three',
        desc: 'Another cool project. Replace this with something you have built or are building.',
        tags: ['React', 'REST API'],
        repo: 'https://github.com/yourusername',
        live: '#',
    },
]
</script>

<template>
    <section id="projects" class="py-24 border-t border-green-900/40">
        <div class="max-w-5xl mx-auto px-6">

            <p class="font-mono-custom text-green-500 text-sm mb-2 tracking-widest">&gt; ls ./projects</p>
            <h2 class="text-3xl font-bold text-white mb-12">Things I've built</h2>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div v-for="project in projects" :key="project.name"
                    class="border border-green-900/60 bg-green-950/10 rounded-lg p-6 flex flex-col justify-between hover:border-green-600/50 transition-colors duration-300 group">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <span class="font-mono-custom text-green-400 font-bold text-sm">{{ project.name }}</span>
                            <span class="text-gray-700 group-hover:text-green-700 transition-colors">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z" />
                                </svg>
                            </span>
                        </div>
                        <p class="text-gray-500 text-sm leading-relaxed mb-4">{{ project.desc }}</p>
                        <div class="flex flex-wrap gap-2 mb-4">
                            <span v-for="tag in project.tags" :key="tag"
                                class="font-mono-custom text-xs px-2 py-0.5 bg-green-950/50 text-green-600 border border-green-900/50 rounded">
                                {{ tag }}
                            </span>
                        </div>
                    </div>
                    <div class="flex gap-3 mt-2">
                        <a :href="project.repo" target="_blank"
                            class="font-mono-custom text-xs text-gray-500 hover:text-green-400 transition-colors">
                            / repo
                        </a>
                        <a :href="project.live" target="_blank"
                            class="font-mono-custom text-xs text-gray-500 hover:text-green-400 transition-colors">
                            / live
                        </a>
                    </div>
                </div>
            </div>

        </div>
    </section>
</template>